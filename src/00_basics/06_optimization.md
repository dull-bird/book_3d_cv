...
> [!NOTE]
> **预计阅读时间**：40 分钟 · **前置知识**：基础篇第 01-05 节
>
...
> [!TIP]
> 假设数据里 30% 是错配，采样 4 个点，要想 99% 把握至少有一次全是好点——需要多少次尝试？公式告诉你答案。
...
$$\min_{P^i, X_j} \sum_{i,j} d(P^i X_j, x^i_j)^2$$
...
> [!TIP]
> 几千个未知数的优化问题看起来不可能，但因为相机和点之间的交互是"稀疏的"（不是每对相机-点都相关），可以用线性代数技巧把大问题分解成几个小问题。这就是 BA 在实践中有用的原因。
...
### 6.3.4 Bundle Adjustment 在实践中的工具
...
- 本书内：[[04 多视图几何入门]]（F 矩阵的 8 点法就是 DLT 的具体应用）、[[01 相机模型]]（BA 中优化的相机参数来源）
...
七章下来，你手里已经有了一套完整的数学工具：

| 章节 | 你学到了 | 核心公式 / 概念 |
|------|---------|---------------|
| 01 相机模型 | 世界如何变成图像 | $P = K[R \mid t]$ |
| 02 坐标系转换 | 在不同参考系之间穿梭 | 世界→相机→图像→像素、PnP |
| 03 投影几何 | 透视变换的数学语言 | 齐次坐标、单应矩阵 $H$ |
| 04 多视图几何 | 两张图之间的几何关系 | $x'^T F x = 0$、本质矩阵 $E$、双目标定 |
| 05 深度表示 | 三维信息如何编码 | 深度图、视差图、点云 |
| 06 优化基础 | 有噪声时如何找最优解 | DLT、RANSAC、BA |

这套工具足以让你从多张 2D 图像重建出 3D 场景——这就是模块 A/B/C 要做的事情。

但有一个前提：**你首先需要高质量的输入数据**。在研究中，你可以从公开数据集下载标定好的图像序列；在实践中，你得自己选择传感器、部署硬件、面对噪声和光照变化。

这时一个自然的问题是：**这些数学原理，在物理世界里是怎么被实现的？**

答案就在下一节。不同的 3D 传感器用不同的物理原理——结构光三角测量、飞行时间相位分析、激光脉冲计时——实现了同一个目标：获取深度。理解这些硬件，不仅能帮你在实际项目中选型，更能让你在进入后续算法模块时，清楚这些算法是为怎样的数据特性设计的。

> [!TIP]
> **两条路从基础篇分岔**：
> - **主动传感**（下一节：3D 相机与传感器）——硬件直接测量深度，用物理替代部分数学
> - **被动重建**（模块 A/B/C）——从图像间接推断深度，用数学弥补物理的不足
>
> 两条路最后殊途同归：都是把三维世界装进计算机的表示里。
> [!NOTE]
> **预计阅读时间：40 分钟** · **前置知识：基础篇第 01-05 节**
>
> **读完本节后，你可以：理解 DLT、几何误差、RANSAC、Bundle Adjustment 的基本原理，知道何时用何种方法，能用 NumPy 实现 DLT 和简单 RANSAC。**
