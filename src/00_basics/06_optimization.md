...
**通俗解读**：假设数据里 30% 是错配，采样 4 个点，要想 99% 把握至少有一次全是好点——需要多少次尝试？公式告诉你答案。
...
**通俗解读**：几千个未知数的优化问题看起来不可能，但因为相机和点之间的交互是"稀疏的"（不是每对相机-点都相关），可以用线性代数技巧把大问题分解成几个小问题。这就是 BA 在实践中有用的原因。
