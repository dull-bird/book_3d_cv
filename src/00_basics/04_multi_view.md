# 04 多视图几何入门：两张照片如何重建三维世界

> [!NOTE]
> **预计阅读时间**：55 分钟 · **前置知识**：本篇第 01 节（相机模型）、第 02 节（坐标系转换）、第 03 节（投影几何）
>
> **读完本节后，你可以**：写出对极约束 $x'^T F x = 0$，理解 F 矩阵 7 个自由度从哪来，用 8 点法从两张图的对应点算出 F，区分 F 和 E（本质矩阵），理解双目标定的完整流程，并理解从两张照片重建三维场景的完整管线。
...
### 4.1.4 Mini Case：用 OpenCV 在一对立体图像上画对极线
...
其中 $C$ 是相机中心（满足 $PC = 0$），$P^+$ 是 $P$ 的伪逆，$P^+ x$ 是射线的方向向量。这条射线在右图中的投影——$P'P^+ x$ 和 $P'C = e'$，恰好定义一个对极点 $e'$。$x'$ 必须落在 $e'$ 和 $P'P^+ x$ 的连线上——这条线就是对极线 $l'$（H&Z section 9.1, p.239-241）。
...
> [!TIP]
> 基础矩阵 $F$ 就像一个两口之家的"户口本"——它编码了两个相机之间的所有几何关系。你能从中读出：(1) 每张图的"盲区"（对极点，零空间方向）；(2) 任意一个点在另一张图中对应的"搜索线"（对极线）；(3) 两个相机之间的相对射影关系。而且这一切都不需要知道相机的焦距、主点等参数——只要有足够多的匹配点对就能算出来。

### 4.2.4 本质矩阵 $E$——戴了眼镜的 F
...
> [!TIP]
> F 和 E 的关系就像"近视眼看世界"和"戴了眼镜看世界"。F 是未校准相机的产物——你不知道焦距、主点等参数，所以 F 含混一些（7 DOF）。E 是校准后的产物——你知道 K 和 K'，可以从像素坐标反推出归一化坐标，得到的 E 干净很多（5 DOF）。最少只需 5 个点对应就能唯一确定 E，而 F 最少要 7 个。
...
下面的 Code Lens 会用 OpenCV 的 `triangulatePoints` 函数做三角测量，同时自动处理正深度筛选。
...
### 4.2.9 Code Lens：用 NumPy 实现归一化 8 点算法
...
### 4.3.3 双目标定：从棋盘格到矫正图像（工程实践）

前面从 $F$ 恢复 $E$ 再恢复 $R,t$ 的方法（§4.2.5）在纯视觉场景下工作，但它有两个局限：

1. **尺度不确定**：$t$ 只有方向，没有绝对长度——你不知道两台相机相距 10cm 还是 1m。
2. **精度有限**：$F$ 是从特征匹配点估计的，匹配误差会传递到 $R,t$。

在工程实践中，双目相机的相对位姿通常通过**双目标定（Stereo Calibration）**获得：同时拍摄棋盘格，联合优化两个相机的内参和相对位姿。
...
棋盘格图像对（左+右）× N 张
    ↓
分别标定左右相机 → K1, dist1, K2, dist2
    ↓
双目标定 stereoCalibrate → R, t（左→右的相对位姿）
    ↓
立体矫正 stereoRectify → R1, R2, P1, P2, Q
    ↓
计算映射 initUndistortRectifyMap + remap → 矫正后的图像对
```
...
# 1. 分别标定左右相机（用第 02 节的棋盘格方法）
# 得到：K1, dist1, K2, dist2

# 2. 双目标定：同时用左右图像对优化 R, t
# objpoints: 3D 棋盘格角点（world coords，单位 mm 或 m）
# imgpointsL, imgpointsR: 左右图对应的 2D 角点
retval, K1, dist1, K2, dist2, R, T, E, F = cv2.stereoCalibrate(
    objpoints, imgpointsL, imgpointsR,
    K1, dist1, K2, dist2,
    image_size,
    flags=cv2.CALIB_FIX_INTRINSIC  # 固定内参，只优化 R,t
)

# 3. 立体矫正：计算两相机到共同矫正平面的旋转和投影矩阵
...
# 4. 计算像素重映射表（只做一次，后续帧复用）
map1L, map2L = cv2.initUndistortRectifyMap(
    K1, dist1, R1, P1, image_size, cv2.CV_16SC2)
map1R, map2R = cv2.initUndistortRectifyMap(
    K2, dist2, R2, P2, image_size, cv2.CV_16SC2)
...
**为什么 `stereoCalibrate` 比分别标定 + 算 $F$ 更可靠？**

| 方面 | 从 $F$ 恢复 $R,t$ | 双目标定 `stereoCalibrate` |
|------|------------------|---------------------------|
| **尺度** | 不确定（只能恢复方向） | 确定（棋盘格方格大小已知） |
| **精度** | 受特征匹配误差影响 | 角点检测亚像素级，联合优化 |
| **内参** | 假设已标定且固定 | 可同时精修内参 |
| **适用场景** | 无标定物、纯视觉 SLAM | 有标定物、工业双目系统 |

> **模块 B 的前置知识**：如果你计划阅读「双目立体匹配」模块，建议先完成一次完整的双目标定流程——矫正后的图像对是立体匹配的输入，标定质量直接决定视差图的精度上限。

> **H&Z 对应**：Ch.11 "Computation of the Fundamental Matrix $F$ and Image Rectification"（p.279-286）讨论了立体矫正的理论基础。双目标定的联合优化框架见 OpenCV 文档 `stereoCalibrate`。

---

### 4.3.4 立体矫正（Stereo Rectification）
...
# 注意：这里的 R, T 来自双目标定（§4.3.3）或从 E 矩阵分解（§4.2.5）
...
| 1981 | H. C. Longuet-Higgins, "A Computer Algorithm for Reconstructing a Scene from Two Projections", *Nature* | **本质矩阵的原始提出**——首次揭示了校准后两视图之间 $3 \times 3$矩阵约束，5 DOF 的性质 |
...
1. 假设你的手机相机已经标定（可以用第 02 节的标定方法得到 K 矩阵）。
