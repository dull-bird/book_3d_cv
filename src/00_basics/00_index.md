...
| 节          | 内容                                | 核心收获                | 阅读时间  |
| ---------- | --------------------------------- | ------------------- | ----- |
| 01 相机模型    | 针孔相机、内参 K、畸变模型、透视/仿射/移轴相机      | 理解相机如何把三维点映射到像素 | 40 分钟 |
| 02 坐标系转换   | 世界→相机→图像→像素四坐标系、标定、PnP（已知 3D+2D 求位姿） | 不再被"各种坐标系"绕晕，掌握标定与位姿估计 | 50 分钟 |
| 03 投影几何    | 齐次坐标、无穷远线、单应矩阵 H、变换分层             | 掌握"透视变换"的数学语言       | 45 分钟 |
| 04 多视图几何入门 | 对极几何、基础矩阵 F、本质矩阵 E、双目标定         | 理解两张图像之间的几何约束       | 55 分钟 |
| 05 深度表示    | 深度图、视差图、点云                        | 知道三维信息有哪几种"存法"      | 30 分钟 |
| 06 优化基础    | DLT、几何误差、RANSAC、Bundle Adjustment | 理解 3D 视觉中"为什么需要优化"  | 40 分钟 |
| 07 3D 相机与传感器 | 消费级/工业级 3D 传感器、ToF、LiDAR 原理与选型 | 理解"数学如何被硬件实现"      | 35 分钟 |

> 建议按顺序阅读。第 01 和 02 节是后续所有内容的基础——01 建立投影直觉，02 建立坐标系直觉；如果你已有一定基础，第 05 节可以快速扫读。第 07 节是后面实战模块的"硬件背景"，建议在进入模块 A/B/C 前通读一遍。
...
- **第 07 节（3D 相机与传感器）** 回答了一个实践问题：这些数学工具需要什么样的输入数据？不同的 3D 传感器如何用不同的物理原理实现同一个目标——获取深度？读完硬件，再进入算法模块，你会更清楚"为什么这个算法这样做"。
...
> 这个案例贯穿基础篇全部七节的知识。完成基础篇学习后，你应该能独立完成这个任务。
