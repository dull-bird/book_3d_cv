...
第二步：倾斜旋转改变了 Z 坐标（归一化坐标的齐次第三分量不再是 1），需要通过**透视校正矩阵**把坐标从倾斜平面"拉回"到传感器平面。这个矩阵的元素 $R_{13}$、$R_{23}$、$R_{33}$ 直接取自 $R(\tau_x, \tau_y)$——它们不是新的自由参数。整个过程只引入了两个额外参数：$\tau_x$ 和 $\tau_y$。

> [!TIP]
> 为什么这个矩阵长这样？想象你把一张纸（传感器）倾斜一个角度。纸上的一个点在三维空间中的位置变了——它的 Z 坐标不再为 1，X 和 Y 坐标也随之缩放。那个带 $R_{33}$ 和 $-R_{13}$ 的矩阵，做的就是"把倾斜平面上的点，映射回正对传感器的视图"。本质上是一次**透视除法前的坐标变换**。
