...
现在我们可以回答本章的核心问题：**为什么我们可以用针孔模型？**

真实相机有镜头、有光圈、有景深。**当光圈收得非常小（f/16、f/32……）时，镜头的行为趋近于一个针孔**。原因很简单：

- 光圈极大时，物体上一个点发出的光线可以从镜头表面的各个位置进入，如果传感器恰好不在完美焦平面上，这个点就会变成一个弥散圆——模糊了。
- 光圈极小时，只有近乎平行的一束光能穿过孔径，无论传感器在什么位置，这个点始终是一个很小的光斑——**近似一个点**。

在极限情况下（光圈趋近于无穷小），光线只能从唯一的一条路径穿过——**从三维点出发 → 穿过小孔 → 直抵成像平面**。此时：

1. **没有景深概念**——针孔下所有距离的物体都同样"清晰"（实际上是同样模糊，受限于衍射）
2. **所有光线都是直线**——没有镜头畸变、没有色差
3. **投影关系变成纯粹的相似三角形**——这正是针孔模型数学简单性的来源

**针孔模型的代价**：进光量极小（因为孔径极小），需要很长的曝光时间或极高的 ISO，在现实摄影中几乎不可用。但对于计算机视觉来说，**这个几何简化是无价的**——它让我们能用简单的线性代数描述三维世界到二维图像的映射，而不必处理真实镜头的非线性光学。

> **人话总结**：真实相机的镜头是一组复杂的光学元件，会让光线弯折、汇聚、产生畸变。但当光圈收得非常小时，大多数"花式光线"被挡住了，只剩下近乎直线穿过中心的那一束——整台相机退化成一个针孔。这个针孔的几何行为正是我们后面所有公式（投影矩阵、对极约束、三角测量……）的物理基础。

后续标定章节会讲到畸变参数——它们正是用来修正"真实镜头偏离针孔模型"的部分。所以整个故事是这样的：**先用针孔模型建立几何关系，再用畸变参数修正光学误差。** 一套完整的"先简化，后修正"的工程方案。
