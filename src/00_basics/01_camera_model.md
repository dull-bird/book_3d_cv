...
**含畸变的完整公式**——归一化 → 畸变 → $K$ 矩阵，三步合成一行：

$$
\boxed{\,x_{pixel} = K \cdot \operatorname{dist}\!\left( \frac{X_{cam}}{Z} \right)\,}
$$

$X_{cam}/Z = (X/Z,\; Y/Z)^T$ 是归一化坐标，$\operatorname{dist}(\cdot)$ 包含径向畸变（$k_1,k_2,k_3$）和切向畸变（$p_1,p_2$）。畸变必须在归一化平面施加（与传感器分辨率无关），$K$ 只负责最后的缩放和偏移。
...
倾斜旋转后 $\tilde{z} \neq 1$。**能不能简单除以 $\tilde{z}$？** 不行。我们用一个具体例子来看为什么。

设仅绕 Y 轴倾斜 $\tau_y$（$\tau_x = 0$）。代入 $R$：

$$R = \begin{bmatrix} \cos\tau_y & 0 & -\sin\tau_y \cr 0 & 1 & 0 \cr \sin\tau_y & 0 & \cos\tau_y \end{bmatrix}$$

旋转后：$\tilde{x} = x'\cos\tau_y - \sin\tau_y$，$\tilde{y} = y'$，$\tilde{z} = x'\sin\tau_y + \cos\tau_y$。

**方案 A：简单除以 $\tilde{z}$（传感器旋转模型）**

$$x''_A = \frac{\tilde{x}}{\tilde{z}} = \frac{x'\cos\tau_y - \sin\tau_y}{x'\sin\tau_y + \cos\tau_y}$$

在图像中心（$x'=0$），$x''_A = -\sin\tau_y / \cos\tau_y = -\tan\tau_y \neq 0$。**图像中心偏移了。** 这意味着简单除法把倾斜当成"传感器转了"——主点位置变了。但镜头倾斜时，**光轴方向没变**，主点应该还在原位。

**方案 B：透视校正矩阵（OpenCV 模型）**

$$x''_B = \frac{R_{33}\tilde{x} - R_{13}\tilde{z}}{\tilde{z}} = \frac{(\cos\tau_y)(x'\cos\tau_y - \sin\tau_y) - (-\sin\tau_y)(x'\sin\tau_y + \cos\tau_y)}{x'\sin\tau_y + \cos\tau_y}$$

分子展开：
$$\begin{aligned} &= (\cos^2\tau_y \cdot x' - \cos\tau_y\sin\tau_y) + (\sin^2\tau_y \cdot x' + \sin\tau_y\cos\tau_y) \\ &= (\cos^2\tau_y + \sin^2\tau_y)x' + (-\cos\tau_y\sin\tau_y + \sin\tau_y\cos\tau_y) \\ &= 1 \cdot x' + 0 = x' \end{aligned}$$

所以：

$$x''_B = \frac{x'}{x'\sin\tau_y + \cos\tau_y}$$

在图像中心（$x'=0$），$x''_B = 0$——**主点没有偏移。** 常数项 $-\sin\tau_y$ 被 $-R_{13}$ 消掉了。

> **透视校正矩阵做了什么？** 它把旋转引入的"平移分量"（$-R_{13}, -R_{23}$）从分子中消掉，使得图像中心始终保持在 $(0,0)$。剩下的分母 $R_{31}x' + R_{32}y' + R_{33}$ 保留——这是一个**纯透视扭曲**：图像左边和右边被不等比例缩放（因为分母随 $x'$ 变化），但中心位置不动。

推广到绕 X、Y 双轴倾斜的一般情况，利用旋转矩阵的正交性 $R^{-1}=R^T$，同样可证：

$$x'' = \frac{R_{22}x' - R_{12}y'}{R_{31}x' + R_{32}y' + R_{33}}, \quad y'' = \frac{-R_{21}x' + R_{11}y'}{R_{31}x' + R_{32}y' + R_{33}}$$

分子中的常数项全部抵消——这就是透视校正矩阵的三个元素 $R_{33}, R_{13}, R_{23}$ 的几何意义。

> [!TIP]
> **一句话**：倾斜旋转让图像又转又移，透视校正矩阵把"移"消掉、只保留"扭"。整个过程只引入 $\tau_x, \tau_y$ 两个参数——校正矩阵的所有元素都取自同一个 $R$。
